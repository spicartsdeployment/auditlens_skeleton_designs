...
import { Briefcase, Search, Plus, ChevronRight, CheckCircle2, AlertTriangle, Download, ChevronDown, FileSpreadsheet } from 'lucide-react'
...
const CLIENT_TEMPLATES = [
  {
    ...
    desc: 'Full onboarding workbook (Excel)',
    filename: 'Client_Onboarding_Template.csv',
    headers: [
      'Legal Name', 'Trade Name', 'PAN', 'GSTIN', 'Contact Name', 'Contact Email',
      'Contact Phone', 'Address', 'State', 'GST Enabled', 'TDS Enabled', 'Audit Enabled',
    ],
  },
  {
    label: 'Bulk Import Template',
    desc: 'Import multiple clients at once',
    filename: 'Client_Bulk_Import_Template.csv',
    headers: [
      'Legal Name', 'Trade Name', 'PAN', 'GSTIN', 'Contact Email', 'Contact Phone',
      'State', 'GST Enabled', 'TDS Enabled', 'Audit Enabled',
    ],
  },
] as const

function downloadClientTemplate(filename: string, headers: readonly string[]) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto" style={{ background: G.canvas }}>
      <PageHeader title="Client Management" sub={`${(clients as Client[]).filter(c => c.is_active).length} active clients`}>
        ...
        <DarkBtn onClick={() => toast.info('Import client template — upload CSV from Download Templates')}><Plus className="h-3.5 w-3.5" />Import Client Template</DarkBtn>
        <div className="relative">
          <button
            onClick={() => setTemplateMenuOpen(o => !o)}
            className="flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-semibold transition-all active:scale-[0.98]"
          ...
          {templateMenuOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 z-50 w-72 rounded-2xl overflow-hidden"
                style={{ background: G.white, border: `1px solid ${G.border}`, boxShadow: '0 8px 20px -2px rgba(15,23,42,0.12)' }}>
                <div className="px-4 py-3" style={{ borderBottom: `1px solid ${G.border}` }}>
                  <p className="text-xs font-bold" style={{ color: G.primary }}>Download Templates</p>
                  <p className="text-[10px] mt-0.5" style={{ color: G.icon }}>Fill and import via Import Client Template</p>
                </div>
                <div className="py-1">
                  {CLIENT_TEMPLATES.map(tmpl => (
                    <button key={tmpl.filename}
                      className="flex w-full items-center gap-3 px-4 py-2.5 transition-colors hover:bg-slate-50"
                      onClick={() => {
                        downloadClientTemplate(tmpl.filename, tmpl.headers)
                        toast.success(`Downloaded ${tmpl.label}`)
                        setTemplateMenuOpen(false)
                      }}>
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg" style={{ background: G.canvas }}>
                        <FileSpreadsheet className="h-3.5 w-3.5" style={{ color: G.secondary }} />
                      </div>
                      <div className="text-left">
                        <p className="text-xs font-semibold" style={{ color: G.primary }}>{tmpl.label}</p>
                        <p className="text-[10px]" style={{ color: G.icon }}>{tmpl.desc}</p>
                      </div>
                    </button>
                  ))}
                </div>

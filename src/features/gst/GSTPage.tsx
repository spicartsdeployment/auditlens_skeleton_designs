...
import {
  ...
  BarChart2, Send, Save, ArrowLeftRight, X,
} from 'lucide-react'
...
// GSTR-3B sheets — Section 6.1 updated: no Cess col, added Cess/Interest/Late Fee rows
const GSTR3B_SHEETS: SheetDef[] = [
  ...
  {
    // Section 6.1 updated: removed Cess col, added Additional Cash Required, added Cess/Interest/Late Fee rows
...
const RECON_TYPES = [
  { key: 'gstr3b_vs_gstr1',  label: 'GSTR-3B vs GSTR-1',   icon: '📊', desc: 'Compare outward liability declared in 3B against invoices in GSTR-1' },
  { key: 'gstr3b_vs_gstr2b', label: 'GSTR-3B vs GSTR-2B',  icon: '📥', desc: 'Verify ITC claimed in 3B matches auto-populated 2B data' },
  { key: 'books_vs_gstr1',   label: 'Books vs GSTR-1',      icon: '📚', desc: 'Reconcile books of accounts outward supplies against filed GSTR-1' },
  { key: 'books_vs_gstr2b',  label: 'Books vs GSTR-2B',     icon: '🔗', desc: 'Match purchase register with GSTR-2B auto-drafted credit' },
  { key: 'sales_recon',      label: 'Sales Reconciliation', icon: '💹', desc: 'Detailed reconciliation of all sales transactions period-wise' },
  { key: 'purchase_recon',   label: 'Purchase Reconciliation', icon: '🛒', desc: 'Detailed reconciliation of purchase transactions and ITC eligibility' },
]

// ─── Recon mock rows ─────────────────────────────────────────────────────────
type ReconRow = { id: string; inv: string; date: string; gstin: string; party: string; books: string; portal: string; diff: string; status: 'mismatched'|'partial'|'matched'; editing?: boolean }
                ...
                {row.map((cell, ci) => (
                  <td key={ci} className="px-3 py-2 whitespace-nowrap">
                    {editing === ri
                      ...
                      : <span style={{ color: ci === 0 ? G.primary : G.secondary, fontWeight: ci === 0 ? 500 : 400,
                          fontFamily: ci > 3 ? 'monospace' : 'inherit' }}>{cell}</span>
                    }
                  </td>
                ))}
                {showActions && (
            ...
            <tbody>
              {[
                ...
                <tr key={i} style={{ borderBottom: `1px solid ${G.border}`, fontWeight: i === 4 ? 700 : 400 }}>
                  {r.map((c, j) => <td key={j} className="px-3 py-2 whitespace-nowrap" style={{ color: G.primary, fontFamily: j > 0 ? 'monospace' : 'inherit' }}>{c}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {[
                ...
                <tr key={i} style={{ borderBottom: `1px solid ${G.border}` }}>
                  {r.map((c, j) => <td key={j} className="px-3 py-2 whitespace-nowrap"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {([
          ...
          <div key={s.label} className="rounded-2xl border p-4" style={{ background: G.white, borderColor: G.border }}>
            ...
            <p className="text-3xl font-bold tabular-nums" style={{ color: s.color === G.icon ? G.primary : s.color }}>{s.value}</p>
          </div>
        ))}
      </div>
...
function ClientsTab({ clients }: { clients: any[] }) {
  ...
  const toggle = (id: number) => setShown(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n })
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => toggle(c.id as number)}
                          ...
                          {visible ? <EyeOff className="h-3.5 w-3.5" style={{ color: G.secondary }} />
...
function ReturnsTab({ clients }: { clients: any[] }) {
  ...
  const showGSTR1Export = returnType !== 'GSTR-1' && returnType !== 'GSTR-1A' && returnType !== 'GSTR-3B'
  ...
  const resetPrep = () => { setPrepMode('none'); setLoaded(false); setActiveSheet(''); setShowSummary(false); if (fileRef.current) fileRef.current.value = '' }
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => { setLoading(false); setLoaded(true); setActiveSheet(sheets[0]?.key ?? ''); toast.success(`${returnType} data loaded — ${sheets.length} sections`) }, 1800)
  }

  const handlePrepOnline = () => { setLoaded(true); setActiveSheet(sheets[0]?.key ?? ''); toast.info('Online preparation mode — enter data manually') }
    ...
    <div className="space-y-4">
      ...
      <ContentCard>
        <div className="p-4 space-y-4">
          {/* Row 1: Client → GSTIN → FY → Month → Return Type */}
          ...
          {canPrepare && !loaded && (
            <div className="flex flex-wrap items-center gap-3 pt-3" style={{ borderTop: `1px solid ${G.border}` }}>
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none rounded-xl px-3 py-2 transition-all"
                ...
                <span className="text-xs font-semibold" style={{ color: prepMode === 'nil' ? '#D97706' : G.secondary }}>
                  Fill Nil Return
                </span>
              </label>
              ...
              {/* Prepare Online */}
              <button
                onClick={() => { setPrepMode('online'); handlePrepOnline() }}
          ...
          {canPrepare && prepMode === 'nil' && !loaded && (
            <div className="rounded-xl p-4 flex items-center justify-between"
              ...
              <div>
                <p className="text-sm font-semibold" style={{ color: '#92400E' }}>Nil Return — {returnType} for {period}</p>
                <p className="text-xs mt-0.5" style={{ color: '#A16207' }}>No transactions to report. Filing a Nil Return will declare zero outward supplies for this period.</p>
              </div>
      ...
      {/* Summary View (full-screen overlay within card) */}
      {showSummary && (
        <ContentCard>
          <div className="p-5">
      ...
      {loaded && !loading && !showSummary && prepMode !== 'nil' && sheets.length > 0 && (
        <div className="space-y-4">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: G.icon }}>
                      {selSheet.rows.length} record{selSheet.rows.length !== 1 ? 's' : ''} · {prepMode === 'online' ? 'Online entry mode' : 'Imported from Excel'}
                    </p>
                  </div>
                </div>
          ...
          {/* Action bar: View Summary | Save as Draft | File & Submit */}
...
function ReconciliationTab({ clients }: { clients: any[] }) {
  ...
  const [retLoaded,    setRetLoaded]    = useState(false)
  const [filter,      setFilter]      = useState<ReconFilter>('all')
  const [rows,        setRows]        = useState<ReconRow[]>(MOCK_RECON)
  ...
  const gstinOptions    = client ? (CLIENT_GSTINS[client] ?? []) : []
  const canSelectType   = !!client && !!gstin && !!year
  const canReconcile    = canSelectType && !!reconType && !!fromMonth && !!toMonth

  const handleClientChange = (v: string) => { setClient(v); setGstin(''); setReconType(''); setBooksLoaded(false); setRetLoaded(false) }

  const handleBooksFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files?.[0]) return
    setLoadingBooks(true)
    toast.info('Uploading books file…')
    setTimeout(() => { setLoadingBooks(false); setBooksLoaded(true); toast.success('Books file uploaded') }, 1500)
  }
  const handleRetFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files?.[0]) return
    setLoadingRet(true)
    toast.info('Uploading return file…')
    setTimeout(() => { setLoadingRet(false); setRetLoaded(true); toast.success('Return file uploaded') }, 1500)
  }
  ...
  const autoMatch = () => {
    setRows(prev => prev.map(r => {
      if ((r.status === 'partial' || r.status === 'mismatched') && r.diff !== '—') {
        ...
        if (diffNum <= 500) return { ...r, status: 'matched', diff: '—' }
      }
      return r
    }))
    toast.success('Auto-matched records within tolerance')
  }
  ...
  const reconDone = booksLoaded && retLoaded && !!reconType
  ...
  return (
    <div className="space-y-4">
      ...
      <ContentCard>
        <div className="p-4 space-y-4">
          {/* Row 1: Client + GSTIN + FY */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <Sel label="Client" value={client} onChange={handleClientChange}
              options={(clients as any[]).filter(c => c.gst_enabled).map(c => ({ value: String(c.id), label: c.legal_name }))} />
            <Sel label="GSTIN" value={gstin} onChange={v => { setGstin(v); setReconType('') }} disabled={!client}
              options={gstinOptions.map(g => ({ value: g, label: g }))} />
            <Sel label="Financial Year" value={year} onChange={setYear} disabled={!gstin}
              options={YEARS.map(y => ({ value: y, label: y }))} />
          </div>

          {/* Row 2: From Month + To Month (period range) */}
          {canSelectType && (
            <div className="grid grid-cols-2 gap-3">
              <Sel label="From Month" value={fromMonth} onChange={setFromMonth}
                options={MONTHS.map(m => ({ value: m, label: m }))} />
              <Sel label="To Month" value={toMonth} onChange={setToMonth}
                options={MONTHS.filter(m => !fromMonth || MONTHS.indexOf(m) >= MONTHS.indexOf(fromMonth)).map(m => ({ value: m, label: m }))} />
            </div>
          )}
        </div>
      </ContentCard>
      ...
      {canSelectType && (
        <div>
          <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: G.secondary }}>Select Reconciliation Type</p>
            ...
            {RECON_TYPES.map(rt => (
              <button key={rt.key} onClick={() => setReconType(rt.key)}
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-bold" style={{ color: reconType === rt.key ? '#1D4ED8' : G.primary }}>{rt.label}</span>
                  {reconType === rt.key && (
                    <span className="ml-auto h-4 w-4 rounded-full flex items-center justify-center"
                      style={{ background: '#2563EB' }}>
                      <Check className="h-2.5 w-2.5 text-white" />
                    </span>
                  )}
                </div>
                <p className="text-[11px]" style={{ color: G.secondary }}>{rt.desc}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      {canReconcile && (
        <ContentCard>
          <div className="p-4">
            <p className="text-xs font-bold mb-3" style={{ color: G.primary }}>Upload Files for Reconciliation</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Books file */}
              <div className="rounded-xl border p-4"
                style={{ background: booksLoaded ? '#F0FDF4' : G.canvas, borderColor: booksLoaded ? '#86EFAC' : G.border }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold" style={{ color: G.secondary }}>Books / Ledger File</span>
                  {booksLoaded && <span className="text-[10px] font-semibold text-green-600">✓ Uploaded</span>}
                </div>
                <p className="text-[11px] mb-3" style={{ color: G.icon }}>Upload your accounting ledger or purchase/sales register (.xlsx)</p>
                <input ref={booksRef} type="file" className="hidden" accept=".xlsx,.xls,.csv" onChange={handleBooksFile} />
                <button onClick={() => booksRef.current?.click()}
                  className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold"
                  style={{ background: booksLoaded ? '#DCFCE7' : G.white, border: `1.5px solid ${booksLoaded ? '#86EFAC' : G.border}`, color: booksLoaded ? '#16A34A' : G.secondary }}>
                  <Upload className="h-3.5 w-3.5" />
                  {loadingBooks ? 'Uploading…' : booksLoaded ? 'Replace Books File' : 'Upload Books File'}
                </button>
              </div>

              {/* Return file */}
              <div className="rounded-xl border p-4"
                style={{ background: retLoaded ? '#F0FDF4' : G.canvas, borderColor: retLoaded ? '#86EFAC' : G.border }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold" style={{ color: G.secondary }}>Return / Portal Data File</span>
                  {retLoaded && <span className="text-[10px] font-semibold text-green-600">✓ Uploaded</span>}
                </div>
                <p className="text-[11px] mb-3" style={{ color: G.icon }}>Upload portal return data (.xlsx) if not available in Returns module</p>
                <input ref={retRef} type="file" className="hidden" accept=".xlsx,.xls,.csv" onChange={handleRetFile} />
                <button onClick={() => retRef.current?.click()}
                  className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold"
                  style={{ background: retLoaded ? '#DCFCE7' : G.white, border: `1.5px solid ${retLoaded ? '#86EFAC' : G.border}`, color: retLoaded ? '#16A34A' : G.secondary }}>
                  <Upload className="h-3.5 w-3.5" />
                  {loadingRet ? 'Uploading…' : retLoaded ? 'Replace Return File' : 'Upload Return File'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {([
              ...
              <button key={s.key} onClick={() => setFilter(s.key as ReconFilter)}
                ...
                <p className="text-2xl font-bold tabular-nums" style={{ color: s.color === G.secondary ? G.primary : s.color }}>{s.cnt}</p>
              </button>
            ))}
          </div>
          ...
          <ContentCard>
            <div className="p-4">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-sm font-semibold" style={{ color: G.primary }}>
                    {RECON_TYPES.find(r => r.key === reconType)?.label} — {fromMonth} to {toMonth} {year}
                  </h3>
                  <p className="text-[11px] mt-0.5" style={{ color: G.icon }}>Edit mismatched rows directly. Records within tolerance auto-move to Matched.</p>
                </div>
                <button onClick={autoMatch}
                  className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold"
                  style={{ background: '#F0FDF4', border: '1px solid #86EFAC', color: '#16A34A' }}>
                  <Check className="h-3.5 w-3.5" />Auto-Match
                </button>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs border-collapse">
                  ...
                  <tbody>
                    {filtered.map((r, i) => {
                      const statusStyle = r.status === 'matched' ? { bg: '#F0FDF4', color: '#16A34A' }
                        : r.status === 'partial'   ? { bg: '#FFFBEB', color: '#D97706' }
                        : { bg: '#FEF2F2', color: '#DC2626' }
                      const isEditing = editId === r.id
                      const canEdit = r.status !== 'matched'
                      return (
                        <tr key={r.id} style={{ borderBottom: `1px solid ${G.border}`, background: isEditing ? '#F8FAFC' : 'transparent' }}
                          ...
                          <td className="px-3 py-2.5 font-mono font-semibold" style={{ color: G.primary }}>
                            {isEditing ? <input value={editBuf.inv ?? ''} onChange={e => setEditBuf(b => ({ ...b, inv: e.target.value }))}
                              className="w-24 rounded px-1.5 py-0.5 text-xs outline-none" style={{ border: '1px solid #0584C7', background: '#EFF8FF' }} />
                              : r.inv}
                          </td>
                          ...
                          <td className="px-3 py-2.5 font-mono">
                            {isEditing ? <input value={editBuf.books ?? ''} onChange={e => setEditBuf(b => ({ ...b, books: e.target.value }))}
                              className="w-20 rounded px-1.5 py-0.5 text-xs outline-none" style={{ border: '1px solid #0584C7', background: '#EFF8FF' }} />
                              : <span style={{ color: G.primary }}>₹{r.books}</span>}
                          </td>
                          <td className="px-3 py-2.5 font-mono">
                            {isEditing ? <input value={editBuf.portal ?? ''} onChange={e => setEditBuf(b => ({ ...b, portal: e.target.value }))}
                              className="w-20 rounded px-1.5 py-0.5 text-xs outline-none" style={{ border: '1px solid #0584C7', background: '#EFF8FF' }} />
                              : r.portal === '—' ? <span style={{ color: '#DC2626', fontWeight: 600 }}>Not found</span>
                          ...
                          <td className="px-3 py-2.5">
                            <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize"
                              ...
                              {r.status === 'matched' ? <Check className="h-3 w-3" />
                                : r.status === 'partial' ? <Minus className="h-3 w-3" />
...
export function GSTPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto" style={{ background: G.canvas }}>
      <PageHeader title="GST Module" sub="Goods & Services Tax filings, credentials and reconciliation">
      ...
      <div className="mb-5 flex gap-1 p-1 rounded-2xl w-fit"
        ...
        {MAIN_TABS.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            ...
            style={{ background: activeTab === t.key ? '#0F172A' : 'transparent', color: activeTab === t.key ? '#FFFFFF' : G.secondary }}>
            {t.label}
          </button>
        ))}
      </div>

import {
  ...
  FileText, CheckSquare, Users, Briefcase,
...
export function TopBar({ alertCount = 0 }: TopBarProps) {
  ...
  return (
    <>
      <header
        className="flex h-12 shrink-0 items-center rounded-2xl border px-4 shadow-sm gap-3"
        style={{
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0 min-w-0">
          ...
          <div className="flex items-center gap-2 min-w-0">
            <div
              className="flex h-8 w-8 items-center justify-center rounded-xl text-white text-sm font-black shadow-sm shrink-0"
              style={{ background: 'linear-gradient(135deg, #0EA5E9 0%, #0284C7 100%)' }}
            >
              AL
            </div>
            <div className="leading-tight min-w-0">
              <p className="text-sm font-black tracking-tight truncate" style={{ color: '#0F172A' }}>
                Audit Lens
              </p>
              <p className="hidden sm:block text-[9px] font-medium truncate" style={{ color: '#94A3B8' }}>
        ...
        <div className="flex-1 flex justify-center px-2">
          <button
            className="hidden md:flex items-center gap-2 h-9 w-full max-w-md rounded-xl border border-slate-200 bg-slate-50 px-3.5 text-xs text-slate-400 hover:border-slate-300 hover:bg-white transition-all"
            onClick={() => toast.info('Search coming soon')}
          >
            <Search className="h-3.5 w-3.5 shrink-0 text-slate-400" />
            <span className="truncate">Search clients, filings, tasks…</span>
            <kbd className="ml-auto text-[10px] border border-slate-200 rounded px-1.5 py-0.5 text-slate-400 bg-white">
              ⌘K
            </kbd>
          </button>
          ...
          {/* Mobile search icon */}
          <button
            className="md:hidden flex h-8 w-8 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100 transition-colors"
            onClick={() => toast.info('Search coming soon')}
            aria-label="Search"
          >
        ...
        {/* ── Right: quick actions + theme + bell + user ── */}
          ...
          <div className="relative hidden sm:block">
            ...
            {quickOpen && (
              <>
                ...
                <div
                  className="absolute right-0 top-full mt-2 z-50 w-64 rounded-2xl border border-slate-200 bg-white/95 shadow-dropdown overflow-hidden"
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 z-50 w-52 rounded-2xl border border-slate-200 bg-white shadow-dropdown overflow-hidden">
                    <div className="px-4 py-3 border-b border-slate-100">
                      ...
                      <span
                        className="mt-2 inline-block rounded-full text-[10px] font-semibold px-2 py-0.5"
